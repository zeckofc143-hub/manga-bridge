...
export default function HomeV2(){
  ...
  const groups=[
    ...
    {
      id:'act',
      label:t('Decidir, calcular e jogar junto','Decide, calculate and play together'),
      help:t('Quando você já sabe o objetivo e quer uma rota, opinião revisada, cálculo ou sistema social.','When you know the goal and want a route, reviewed advice, calculation or social system.'),
      items:[
        ...
        ['#/clans','🏳️',t('Clãs & Social','Clans & Social'),t('Entrada, doações, bônus, cargos, co-op e guerras.','Joining, donations, bonuses, roles, co-op and wars.'),CLAN_RECORDS.length],
  ...
  const quick=[
    ...
    ['#/clans/joining','🏳️',t('Quero entrar num clã','I want to join a clan'),t('Veja requisitos, convites, doações e como avaliar um clã antes de entrar.','See requirements, invites, donations and how to evaluate a clan before joining.')]
  ];
  ...
  return <div className="hv2-page">
    <section className="hv2-hero">
      <div className="hv2-hero-copy">
        ...
        <p>{t('A wiki cobre jogo, mundo, lojas, eventos, quests, clãs e ferramentas. Comece pelo objetivo; detalhes e fontes ficam disponíveis sem lotar a primeira tela.','The wiki covers the game, world, shops, events, quests, clans and tools. Start with your goal; details and sources stay available without crowding the first screen.')}</p>
      ...
      <aside className="hv2-confidence" aria-label={t('Como ler a wiki','How to read the wiki')}>
        <ShieldCheck size={22}/><div><b>{t(`${total}+ entradas organizadas em 13 áreas.`,`${total}+ entries organized across 13 areas.`)}</b><p>{t('Fato, estratégia comunitária, conflito e dado incompleto continuam separados.','Facts, community strategy, conflicts and incomplete data remain separated.')}</p></div>
      </aside>
    </section>
