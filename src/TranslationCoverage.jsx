...
import { translateRawText, useLanguage } from './i18n';
...
const safeNormalize = value => String(value ?? '').replace(/\s+/g,' ').trim();
...
function convertText(raw, language){
  ...
  if(exact.has(safeNormalize(core))) core = exact.get(safeNormalize(core));

  const fragments = language === 'en' ? FRAGMENTS_EN : FRAGMENTS_PT;
  ...
  if(language === 'en'){
    core = core
      ...
      .replace(/^(.+)\s+resultados$/i,'$1 results');
  }

  return `${leading}${core}${trailing}`;
}

function processElement(root, language){
  if(!root) return;
  const handleText = node => {
    if(node.parentElement?.closest?.('[data-no-auto-i18n="true"]')) return;
    const next = convertText(node.nodeValue, language);
    if(next !== node.nodeValue) node.nodeValue = next;
  };

  if(root.nodeType === Node.TEXT_NODE) handleText(root);
  if(root.nodeType === Node.ELEMENT_NODE){
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
    let node;
    while((node = walker.nextNode())) handleText(node);

    root.querySelectorAll?.('[placeholder],[aria-label],[title]').forEach(el => {
      for(const attr of ['placeholder','aria-label','title']){
        if(!el.hasAttribute(attr)) continue;
        const current = el.getAttribute(attr) || '';
        const next = convertText(current, language);
