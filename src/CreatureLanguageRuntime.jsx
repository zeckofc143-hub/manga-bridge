...
import { creatureDescription, creatureName, translateRawText, useLanguage, CREATURE_NAMES } from './i18n';
...
function englishObtain(creature){
  ...
  if(creature.id==='crab') return [
    'Play and win Crab Beach Co-op.',
    'Each valid completion adds progress to the Crab activity bar.',
    'Fill the bar and make sure you have a free creature slot.',
    'Claim the 1★ Crab directly into your army.'
  ];
  if(creature.id==='frog') return [
    'Play Frog Pond Co-op.',
    'Earn valid daily progress for the Frog activity bar.',
    'Fill the bar and make sure you have a free creature slot.',
    'Claim the 1★ Frog directly into your army.'
  ];
  if(creature.id==='christmas-crab') return [
    'Wait for a Christmas event edition where Christmas Crab is available.',
    'Complete the required Christmas Crab / Crab Beach co-op progress.',
    'Obtain the Crab Token required by that event edition.',
    'With a full bar, the token and a free slot, add Christmas Crab directly to the army.'
  ];
  if(creature.captureStatus==='noncapturable') return [
    'This entity cannot be captured or added to the creature army.',
    creature.attraction ? `Encounter location: ${translateLocation(creature.attraction,'en')}.` : 'It appears as part of a map activity, NPC encounter or boss fight.'
  ];
  if(creature.captureStatus==='ally') return [
    'This unit is not captured through the Creatures Chamber.',
    creature.attraction ? `It appears or is obtained at: ${translateLocation(creature.attraction,'en')}.` : 'Follow the activity or colony system associated with this ally.'
  ];
  if(creature.category==='event') return [
    'Wait for the original event or a returning Creature Event / mini-event where this special is available.',
    'Fill the event activity bar required to make the creature appear.',
    'Defeat it and choose Capture if a creature slot is free.'
  ];
  return [
    creature.attraction ? `Use pheromones while meeting this condition: ${translateLocation(creature.attraction,'en')}.` : 'Use pheromones to attract the creature.',
    'Defeat the creature with your soldier ants.',
    'Choose Capture and keep a free creature slot until the process is complete.'
  ];
}
...
function applyCreatureCard(card,language){
  ...
  if(description) description.textContent = creatureDescription(id,creature.description,language);
  ...
  if(variant && creature.variantOf && creature.variantOf!=='—'){
    ...
    variant.textContent = language==='en'?`Variant of ${variantName}`:`Variante de ${variantName}`;
  }
  ...
  if(obtainPreview && steps?.length) obtainPreview.textContent = steps[0];
  const condition = card.querySelector('.ce3-condition span');
  if(condition && creature.attraction) condition.textContent = translateLocation(creature.attraction,language);
}

function applyCreatureDetail(language){
  const id = idFromHref(window.location.hash);
  const creature = BY_ID.get(id);
  const root = document.querySelector('.ce3-detail-page');
  ...
  if(lead) lead.textContent = creatureDescription(id,creature.description,language);
  if(img) img.setAttribute('alt',language==='en'?`${displayName} in Pocket Ants`:`Imagem de ${displayName} em Pocket Ants`);

  const variant = root.querySelector('.ce3-variant b');
  if(variant && creature.variantOf && creature.variantOf!=='—'){
    const variantId = BY_EN_NAME.get(creature.variantOf);
    variant.textContent = variantId ? creatureName(variantId,creature.variantOf,language) : creature.variantOf;
  }

  const stepNodes = [...root.querySelectorAll('.ce3-obtain-panel ol li p')];
  const steps = language==='en' ? englishObtain(creature) : creature.obtain;
  stepNodes.forEach((node,index)=>{ if(steps?.[index]) node.textContent=steps[index]; });

  const location = root.querySelector('.ce3-location span');
  if(location && creature.attraction){
    location.innerHTML = language==='en'
      ? `<b>Condition / location:</b> ${translateLocation(creature.attraction,'en')}`
      : `<b>Condição / local:</b> ${creature.attraction}`;
  }
}

export default function CreatureLanguageRuntime(){
  ...
  useEffect(()=>{
    ...
    const apply=()=>{
      ...
      applyCreatureDetail(language);
    };
    const schedule=()=>{ if(!queued){ queued=true; queueMicrotask(apply); } };
    apply();
    const observer=new MutationObserver(schedule);
    observer.observe(document.body,{subtree:true,childList:true,characterData:false});
    window.addEventListener('hashchange',schedule);
    return ()=>{ observer.disconnect(); window.removeEventListener('hashchange',schedule); };
  },[language]);
  return null;
}
